...
googleyoutube.prototype.getAllVideos = function(options,callback){
	var tokens = this.fetchTokenFromFile();	
	...
	this.refreshToken(tokens,function(err,tokens){
		if(!err){
			...
			youtube.playlistItems.list({
		        auth: that.oauth2Client,
				part: options.part,
				playlistId: "PLlKSJFmSYgWMPz_3ECKBA8YephItgzFi_",
		    },function(err,resp){
		    	if(err){
		    		callback(err,null);	
		    	}else{
...
googleyoutube.prototype.changeVideoStatus = function(options,callback){
	var tokens = this.fetchTokenFromFile();	
